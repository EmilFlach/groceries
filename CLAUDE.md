...
Run: `./kotlin run -m desktopApp` (discover tasks with `./kotlin show`). Hot reload: add `--compose-hot-reload-mode` — a persistent session; stop with **Ctrl-C**, not by closing the window (else the DevTools sidecar orphans: `pkill -f 'apple.awt.application.name=Compose'`). **Validating UI changes: prefer hot reload over rebuilds — it's the default.** A rebuild-per-change loop is slow; instead keep one hot-reload session up and iterate. Compose Hot Reload's MCP server (`take_screenshot`, `get_semantic_tree`, `click`/`type_text`/`reload`/…) is wired up: `./kotlin compose-hot-reload-mcp-server`, registered for Claude Code in `.mcp.json`. It attaches to a running hot-reload session via the pid file, so first start the app with `--compose-hot-reload-mode`, then use the MCP tools (no session running → the server waits, watching `build/hot-reload-app.pid`). Falls back to screenshotting the running desktop app if the session isn't up.

Loop: start once (`./kotlin run -m desktopApp --compose-hot-reload-mode`, background), check `status` for `connected:true`, `click`/`type_text` to the screen under test, then per edit → `reload` → `take_screenshot`. Reuse the session across edits — don't restart per change; state (search text, scroll) survives a reload. **Gotcha:** a string/constant-only edit reports `reloaded:false` / "no changed classes to reload" yet still applies — trust the screenshot, not that message. Scope: this validates **common + JVM/desktop** UI only; platform-specific rendering (`src@android/`, `src@ios/`, `src@wasmJs/`) still needs its own build+run.
